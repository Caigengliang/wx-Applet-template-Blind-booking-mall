...
const defaultConfig = {
  header: {
    ...
    getAuthorization: () =>
      wx.getStorageSync(globalEnum.storageKeys.loginInfo).token == undefined
        ? "Bearer"
        : "Bearer " + wx.getStorageSync(globalEnum.storageKeys.loginInfo).token,
...
function httpLogInfo(url, params, res, method, header) {
  console.log(
    "当前时间：" +
      dateTool.sampleFormatTime(new Date(Date.now())) +
      "\n当前url:" +
      url
  );
...
function successCallback(res, url, params, resolve, reject, method, header) {
  ...
  if (res.statusCode === 200 && res.data.Code != -10003) {
    if (res.data.status === 200) {
      // 成功，Code判断的值根据实际api返回进行修改
      return resolve(res.data.data);
    } else if (res.data.status === 410000 || res.data.status === 410001) {
      wx.showToast({
        title: res.data.msg,
        icon: "none",
      });
  ...
  const headers = Object.assign(
    {
      ...
      "Authori-zation": defaultConfig.header.getAuthorization(), // token 名字
