...
Component({
  ...
  methods: {
    cardReflect() {
      ...
      let sc = showCards;
      if (showCards < 1) sc = 1;
      else if (showCards > cards.filter((item) => item).length)
        sc = cards.filter((item) => item).length;
      this.setData({
        current_z_index: new Array(sc)
          .fill(0)
          .map((_, index) => index + 1)
          .reverse(),
        sc: sc,
    ...
    countCurrentCursor(current_cursor) {
      ...
      if (circling)
        // 如果开启循环
        current_cursor =
          current_cursor + 1 === cards.length ? 0 : current_cursor + 1;
      else current_cursor += 1;
